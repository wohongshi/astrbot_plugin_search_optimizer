...
拦截搜索/抓取工具返回结果，两阶段压缩后再交给主力模型：

```
原始内容（2000字）
  → 第一阶段：规则提取去噪去重（→ 800字）
  → 第二阶段：小模型精简归纳（→ 400字）[可选]
  → 交给主力模型
```

- **不配置预处理模型** → 仅规则提取（零 LLM 调用，压缩率 ~70%）
- **配置预处理模型** → 规则提取 + 小模型精简（压缩率 ~85%）

规则提取先去除噪声和重复内容，小模型再对精简后的内容做归纳总结。
小模型处理的内容更短更干净，效果比直接处理原始内容好很多。
...
| 方案 | 流程 | 压缩率 | 适用场景 |
|------|------|--------|----------|
| 留空 | 规则提取 → 主力模型 | ~70% | 零额外成本 |
| 配置小模型 | 规则提取 → 小模型精简 → 主力模型 | ~85% | Token 敏感，追求高压缩率 |

即使配置了小模型，规则提取也会先运行。
小模型处理的是规则提取后的精简内容，不是原始内容，效果更好。

> **注意**：小模型调用时会自动跳过 AstrBot 人格配置（system_prompt 为空），
> 避免小模型被人格设定影响输出异常。
...
| 指令 | 说明 |
|------|------|
| `/搜索优化器` | 查看运行统计（模式、模型、缓存、压缩率等） |
| `/清除缓存` | 清空全部搜索缓存 |
| `/优化开启` | 开启优化搜索（命中缓存时跳过搜索） |
| `/优化关闭` | 关闭优化搜索 |
| `/小模型开启` | 开启小模型直接回答（需先配置预处理模型） |
| `/小模型关闭` | 关闭小模型直接回答 |
...
第一阶段：规则提取
  └─ 去噪、去重、截断（→ 压缩率 ~70%）
  ↓
第二阶段：小模型精简 [可选，需配置预处理模型]
  └─ 对规则提取后的内容进一步归纳（→ 压缩率 ~85%）
  ↓
压缩后内容交给主力模型
  ↓
主力模型基于精炼内容回答用户
